...
def download_file_from_directory(adls_fsys_name, adls_dir_name):
    try:
        ...
        print(type(downloaded_bytes))

        #data = BytesIO(downloaded_bytes)
        data = json.load(data) 
        #aces = pd.read_json(data)
        aces =  pd.json_normalize(data, 'value')
        aces = aces[['subject', 'start.dateTime', 'end.dateTime']]
        print(aces)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        aces = download_file_from_directory(adls_fsys_name, adls_dir_name)
        #read_and_extract_graph_json()
        upload_file_to_directory(adls_fsys_name, adls_dir_name, df=aces)
