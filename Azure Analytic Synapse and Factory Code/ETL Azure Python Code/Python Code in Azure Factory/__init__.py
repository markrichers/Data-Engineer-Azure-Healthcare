...
def read_csv_to_dataframe(container_client, filename, file_delimiter= ','):
    ...
    df = pd.read_csv(blob_data,delimiter=file_delimiter,index_col=None, skiprows=1, header=None)
    return df
...
def process_relational_data(df):
    ...
    df.columns = ["user_id","device_id","ring_id","mm_level","scr_min","scl","step_count","calibration_lvl","time_iso","time_unix","delcol"]
    df = df.drop(["time_unix","delcol"],axis=1)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        abs_container_name='moodmetric-u1'
        archive_container_name = 'moodmetric-u1-archive'
